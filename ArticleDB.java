...
private static Connection getConnection() {
	...
	try {
		cn = DriverManager.getConnection("jdbc:mysql://localhost:3306/mybase", "root", "");
...
public static void ajouterArticle(Article art) {
	...
	try {
		PreparedStatement ps= cn.prepareStatement("insert into article(nom,prix) values (?,?)");
...
public static Article getArticleById(int id) {
	...
	try {
		PreparedStatement ps = cn.prepareStatement("select * from article where id =? ") ;
	    ps.setInt(1, id);
...
public static void updateArticle(Article art) {
	...
	try {
		...
		PreparedStatement ps = cn.prepareStatement("update article set nom=? , prix=? where id=?");
		ps.setString(1, art.getNom());
		ps.setDouble(2, art.getPrix());
...
public static void supprimerArticle(int id) {
	...
	try {
		...
		PreparedStatement ps = cn.prepareStatement("delete from article where id=?");
